...
// // Create Product (Enhanced with key alias and type conversion)
// exports.createProduct = async (req, res) => {
//   console.log('DEBUG: Full req.body:', req.body); // Remove in prod
//   const { name, category: categoryValue, subCategory: subcategoryValueFromCamel, subcategory: subcategoryValueFromSnake, brand: brandValue, weightQuantity, unit: unitValue, purchasePrice, price, discountPrice, stockQuantity, expiryDate, ingredients, suitableFor, status = 'Active' } = req.body;
...
//     const allFiles = [...imagesFiles, thumbnailFile].filter(f => f);
...
//   if (discountPrice !== undefined && parsedDiscountPrice > parsedPrice) {
//     const allFiles = [...imagesFiles, thumbnailFile].filter(f => f);
//     for (const file of allFiles) {
//       try { await fs.unlink(file.path); } catch { }
//     }
...
//         try { await fs.unlink(file.path); } catch { }
//       }
//       return res.status(400).json({ success: false, msg: `Subcategory not found for value: ${subcategoryValue}` }); // Enriched msg
//     }
...
//     console.log('DEBUG: Ingredients processed:', ingredientsString); // Remove in prod (ignores nutritionFacts)
...
//     };
//     if (discountPrice !== undefined) productData.discountPrice = parsedDiscountPrice;
//     if (stockQuantity !== undefined) productData.stockQuantity = parsedStockQuantity; // Allow 0
//     if (expiryDate) productData.expiryDate = new Date(expiryDate);
//     if (ingredientsString) productData.ingredients = ingredientsString;
//     if (suitableFor) productData.suitableFor = suitableFor;
//     if (images.length > 0) productData.images = images;
//     productData.thumbnail = thumbnail; // Required, so always set
...
//     // Comprehensive cleanup: all images + thumbnail
//     const allFiles = [...imagesFiles, thumbnailFile].filter(f => f);
//     for (const file of allFiles) {
//       try { await fs.unlink(file.path); } catch { }
//     }
//     res.status(500).json({ success: false, msg: 'Server error during product creation' });
//   }
// };


// // Get All Products (List View) - Enhanced with dynamic offers (Fixed projection)
...
//   if (lowStock === 'true') filter.stockQuantity = { $lt: 10 }; // Threshold example

//   try {
//     // Switch to aggregation for offer integration and effective price calc
//     const pipeline = [
//       { $match: filter },
//       // Basic populates via lookup (equivalent to populate)
//       {
//         $lookup: {
//           from: 'categories',
//           localField: 'category',
...
//       // Offer lookup: Get latest active overlapping offer (project only needed fields)
//       {
//         $lookup: {
//           from: 'offers',
...
//             { $sort: { createdAt: -1 } },
//             { $limit: 1 },
//             // Project only discount fields in the offer lookup
//             {
//               $project: {
//                 discountType: 1,
//                 discountValue: 1,
//                 _id: 0 // Exclude offer ID if not needed
//               }
//             }
//           ],
//           as: 'activeOffer'
...
//       // Calculate effective price (using sellPrice; adjust to 'price' if that's your field)
//       {
//         $addFields: {
//           effectivePrice: {
//             $cond: {
...
//                     $subtract: [
//                       { $ifNull: ['$sellPrice', 0] },
//                       { $multiply: [{ $ifNull: ['$sellPrice', 0] }, { $divide: ['$activeOffer.discountValue', 100] }] }
//                     ]
//                   },
//                   else: { $subtract: [{ $ifNull: ['$sellPrice', 0] }, '$activeOffer.discountValue'] }
//                 }
//               },
//               else: { $ifNull: ['$price', 0] } // Or fallback to manual discountPrice if exists
//             }
//           }
//         }
//       },
...
//       // Project: Use exclusion only to avoid mixing (includes all fields + computed ones, hides __v)
//       {
//         $project: {
//           __v: 0
//         }
...
// // Get Product by ID - Enhanced with dynamic offers
...
//       },
//       { $unwind: { path: '$unit', preserveNullAndEmptyArrays: true } },
//       // Offer lookup: Get latest active overlapping offer
//       {
//         $lookup: {
//           from: 'offers',
//           let: { prodId: '$_id', currentDate: new Date() },
...
//         }
//       },
//       // Project to hide __v and include computed field
//       {
//         $project: {
//           __v: 0
//         }
...
//     res.status(500).json({ success: false, msg: 'Server error fetching product' });
//   }
// };

// // Update Product (Similar enhancements: logs, enriched errors, trim)
// exports.updateProduct = async (req, res) => {
//   console.log('DEBUG: Update req.body:', req.body); // Remove in prod
//   const { name, category: categoryValue, subcategory: subcategoryValue, brand: brandValue, weightQuantity, unit: unitValue, purchasePrice, price, discountPrice, stockQuantity, expiryDate, ingredients, suitableFor, status } = req.body;
...
//   const newThumbnail = newThumbnailFile ? newThumbnailFile.path : undefined; // Only if provided

//   // Validation (similar to create, but optional)
//   if (status !== undefined && !['Active', 'Inactive'].includes(status)) {
//     const allNewFiles = [...newImagesFiles, newThumbnailFile].filter(f => f);
...
//       console.log('DEBUG: Update subcategory value:', subcategoryValue); // Remove in prod
//       if (subcategoryValue === null || subcategoryValue === '') {
//         const allNewFiles = [...newImagesFiles, newThumbnailFile].filter(f => f);
//         for (const file of allNewFiles) {
//           try { await fs.unlink(file.path); } catch { }
//         }
//         return res.status(400).json({ success: false, msg: 'Subcategory is required' });
//       } else {
...
//             try { await fs.unlink(file.path); } catch { }
//           }
//           return res.status(400).json({ success: false, msg: `Subcategory not found for value: ${subcategoryValue}` }); // Enriched
//         }
//         subcategory = sub._id;
//       }
//     }

//     let brand = currentProduct.brand;
...
//           try { await fs.unlink(file.path); } catch { }
//         }
//         return res.status(400).json({ success: false, msg: 'Unit is required' });
//       } else {
//         const unitDoc = await findUnitByIdOrName(unitValue);
//         if (!unitDoc) {
//           const allNewFiles = [...newImagesFiles, newThumbnailFile].filter(f => f);
//           for (const file of allNewFiles) {
//             try { await fs.unlink(file.path); } catch { }
...
//     }
//     if (finalPrice <= finalPurchasePrice) {
//       const allNewFiles = [...newImagesFiles, newThumbnailFile].filter(f => f);
//       for (const file of allNewFiles) {
//         try { await fs.unlink(file.path); } catch { }
//       }
...
//     if ((name !== undefined && name !== currentProduct.name) || (brandValue !== undefined && brand !== currentProduct.brand)) {
//       const checkName = name !== undefined ? name : currentProduct.name;
...
//     if (categoryValue !== undefined && category !== currentProduct.category) updateData.category = category;
//     if (subcategoryValue !== undefined && subcategory !== currentProduct.subcategory) updateData.subcategory = subcategory;
//     if (brandValue !== undefined && brand !== currentProduct.brand) updateData.brand = brand;
//     if (unitValue !== undefined && unit !== currentProduct.unit) updateData.unit = unit;
...
//     if (newImages.length > 0) updateData.$push = { images: { $each: newImages } }; // Append new images
//     if (newThumbnail) {
//       // Replace thumbnail and delete old if exists
//       updateData.thumbnail = newThumbnail;
//       if (currentProduct.thumbnail) {
//         try { await fs.unlink(currentProduct.thumbnail); } catch { }
//       }
//     }

//     if (Object.keys(updateData).length === 0 && newImages.length === 0 && !newThumbnail) {
...
//     }
//     res.status(500).json({ success: false, msg: 'Server error updating product' });
//   }
// };
...
//     res.status(500).json({ success: false, msg: 'Server error deleting product' });
//   }
// };
...
exports.createProduct = async (req, res) => {
  console.log('DEBUG: Full req.body:', req.body); // Remove in prod
  const { name, category: categoryValue, subCategory: subcategoryValueFromCamel, subcategory: subcategoryValueFromSnake, brand: brandValue, weightQuantity, unit: unitValue, purchasePrice, price, discountPrice, stockQuantity, expiryDate, ingredients, suitableFor, status = 'Active', variations } = req.body;
...
exports.updateProduct = async (req, res) => {
  console.log('DEBUG: Update req.body:', req.body); // Remove in prod
  const { name, category: categoryValue, subcategory: subcategoryValue, brand: brandValue, weightQuantity, unit: unitValue, purchasePrice, price, discountPrice, stockQuantity, expiryDate, ingredients, suitableFor, status, variations: incomingVariations, variationOperation, variationIndex } = req.body;
