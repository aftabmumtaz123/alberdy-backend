  ...
  try {
    ...
    if (!customerId || !products || !summary) {
      return res
        ...
        .json({
          status: false,
          message: "Customer ID, products, and summary are required",
        });
    }
    ...
    if (typeof otherCharges !== "number" || otherCharges < 0) {
      return res
        ...
        .json({
          status: false,
          message: "otherCharges must be a non-negative number",
        });
    }
    ...
    if (!customer)
      return res
        .status(400)
        .json({ status: false, message: "Invalid customer" });
    ...
    for (let prod of products) {
      if (!prod.variantId || !prod.quantity || !prod.price || !prod.unitCost) {
        return res
          ...
          .json({
            status: false,
            message:
              "Each product must have variantId, quantity, price, and unitCost",
          });
      }
      if (
        prod.quantity < 1 ||
        prod.price < 0 ||
        prod.unitCost < 0 ||
        (prod.taxPercent && prod.taxPercent < 0)
      ) {
        return res
          .status(400)
          .json({
            status: false,
            message: "Quantity, price, unitCost, and taxPercent must be valid",
          });
      }
      if (prod.taxType && !["Inclusive", "Exclusive"].includes(prod.taxType)) {
        return res
          ...
          .json({
            status: false,
            message: "taxType must be Inclusive or Exclusive",
          });
      }
      ...
      if (
        ...
        variant.stockQuantity < prod.quantity
      ) {
        return res
          .status(400)
          .json({
            status: false,
            message: `Insufficient stock or invalid variant: ${prod.variantId}`,
          });
      }
      const taxAmount =
        ((prod.price * prod.quantity * (prod.taxPercent || 0)) / 100) *
        (prod.taxType === "Exclusive" ? 1 : 0);
    ...
    if (discount > subTotal)
      return res
        .status(400)
        .json({ status: false, message: "Discount cannot exceed subtotal" });
    if (grandTotal < 0)
      return res
        .status(400)
        .json({ status: false, message: "Grand total cannot be negative" });
    if (summary.subTotal && summary.subTotal !== subTotal) {
      return res
        .status(400)
        .json({
          status: false,
          message: "Provided subTotal does not match calculated subTotal",
        });
    }
    if (summary.grandTotal && summary.grandTotal !== grandTotal) {
      return res
        ...
        .json({
          status: false,
          message: "Provided grandTotal does not match calculated grandTotal",
        });
    }
    ...
    if (payment) {
      if (typeof payment.amountPaid !== "number" || payment.amountPaid < 0) {
        return res
          ...
          .json({
            status: false,
            message: "payment.amountPaid must be a non-negative number",
          });
      }
      if (
        payment.type &&
        !["Cash", "Card", "Online", "BankTransfer"].includes(payment.type)
      ) {
        return res
          .status(400)
          .json({ status: false, message: "Invalid payment type" });
      }
    }
    ...
    for (let prod of validatedProducts) {
      await Variant.findByIdAndUpdate(prod.variantId, {
    ...
    const populatedSale = await Sale.findById(sale._id)
      ...
      .populate({
        ...
        select:
          "sku attribute value unit purchasePrice price discountPrice stockQuantity expiryDate weightQuantity image",
        populate: [
    ...
    res.status(201).json({
      ...
      data: {
        ...
        products: populatedSale.products.map((product) => ({
          ...
          image:
            product.variantId?.product?.thumbnail ||
...
data: sales.map((sale) => {
  ...
  return {
    ...
    customer: {
      ...
      name: sale.customerId?.name || 'Walk-in',
...
exports.updateSale = async (req, res) => {
  try {
    ...
    if (!sale || sale.isDeleted)
      return res.status(404).json({ status: false, message: "Sale not found" });
    ...
    for (let prod of products) {
      ...
      const oldProduct = sale.products.find(
        (p) => p.variantId.toString() === prod.variantId.toString()
      );
      const availableStock = variant
        ? variant.stockQuantity + (oldProduct ? oldProduct.quantity : 0)
        : 0;
      if (
        !variant ||
        variant.status === "Inactive" ||
    ...
    for (let oldProd of oldProducts) {
      const newProd = newProducts.find(
        (p) => p.variantId.toString() === oldProd.variantId.toString()
      );
      const diff = newProd
        ? oldProd.quantity - newProd.quantity
        : oldProd.quantity; // Positive diff = restore stock
      if (diff !== 0) {
        await Variant.findByIdAndUpdate(oldProd.variantId, {
    ...
    for (let newProd of newProducts) {
      await Variant.findByIdAndUpdate(newProd.variantId, {
...
exports.deleteSale = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ status: true, message: "Sale soft deleted successfully" });
  } catch (error) {
